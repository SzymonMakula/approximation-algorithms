...
pub fn fptas_knapsack(data_set: DataSet, e: f64) -> SolveResult {
    ...
    let (result, items) = _dynamic_programming_knapsack(new_values, weights, data_set.capacity);

    let val = items.into_iter().map(|index| values[index]).sum::<i64>();

    println!("computed {}, summary {} and ", max_value, val);

    SolveResult {
        result: val,
        ratio: val as f64 / data_set.optimal_value as f64,
        data_set,
